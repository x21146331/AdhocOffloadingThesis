...
public class NomadicMobility extends MobilityModel {
	...
	public void initialize() {
		...
		/*NodeList adhocList = doc.getElementsByTagName("adhoc");

		for (int i = 0; i < adhocList.getLength(); i++) {
			Node datacenterNode = adhocList.item(i);
			Element datacenterElement = (Element) datacenterNode;
			Element location = (Element)datacenterElement.getElementsByTagName("location").item(0);
			String attractiveness = location.getElementsByTagName("attractiveness").item(0).getTextContent();
			int placeTypeIndex = Integer.parseInt(attractiveness);
			
			expRngList[datacenterList.getLength()+i] = new ExponentialDistribution(SimSettings.getInstance().getMobilityLookUpTable()[placeTypeIndex]);
		}*/
		
		//NodeList mergedList = join(datacenterList,adhocList);
